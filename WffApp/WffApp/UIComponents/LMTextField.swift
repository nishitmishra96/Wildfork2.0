...
//  LuminousFramework
//
//  Created by Himanshu on 03/10/18.
 ...
 */
public protocol LMUnderLinedTextFieldDelegates{
    func didEndCharLimit(_ textField:LMUnderLinedTextField)
}
@IBDesignable
open class LMUnderLinedTextField: UITextField { // swiftlint:disable:this type_body_length
    /**
    ...
    let leftSecureEye = UIButton(type: UIButton.ButtonType.system)
    let editicon = UIButton(type: UIButton.ButtonType.system)
    open override func awakeFromNib() {
        self.changekeyboardRightview()
        //self.tintColor = .app_text_black_Color
        self.tintColor = .black //newLine
        setFont()
    }
    public var typeOftextfield:textFieldtype = .none{
        didSet{
            self.changekeyBoardStyle()
            self.changekeyboardRightview()
        }
    }
    public var maximumNumberOfdigits:Int?
    public var customDelegates:LMUnderLinedTextFieldDelegates?
    public var isTextFieldEnabled:Bool?{
        didSet{
           self.changeEditoptions()
        }
    }
...
//    @objc dynamic open var titleFont: UIFont = UIFont.appFont(size: .smallTextFont)
    @objc dynamic open var titleFont: UIFont = UIFont.systemFont(ofSize: 10.0) //newLine
    ...
    /// A UIColor value that determines the color used for the title label and line when text field is disabled
     @IBInspectable
    @objc dynamic open var disabledColor: UIColor = UIColor(white: 0.6, alpha: 1.0) {
        didSet {
            updateControl()
            updatePlaceholder()
        }
    ...
    /// A UIColor value that determines the text color of the title label when editing
     @IBInspectable
    @objc dynamic open var selectedTitleColor: UIColor = .gray {
        didSet {
            updateTitleColor()
        }
    }
    ...
    fileprivate final func init_textField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        updateColors()
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(LMUnderLinedTextField.editingChanged), for: .editingChanged)
    ...
    fileprivate func updateLineColor() {
        ...
        if !isEnabled {
            ...
            self.editicon.tintColor = lineView.backgroundColor
            self.leftSecureEye.tintColor = lineView.backgroundColor
            self.dropDown.tintColor = lineView.backgroundColor
        }
    }
...
extension LMUnderLinedTextField{
   public enum textFieldtype{
        case phoneNumber,otp,password,none,dropDown,email
    }
    private func didChangevalue(){
        if (maximumNumberOfdigits != nil){
            if (self.text?.count ?? 0 >= maximumNumberOfdigits!){
//                self.text = String(/text?.prefix(maximumNumberOfdigits!))
                //removedLine
            }
        }
    ...
    private func phoneNumberText(){
        let aSet = NSCharacterSet(charactersIn:"0123456789").inverted
        let compSepByCharInSet = self.text!.components(separatedBy: aSet)
        var numberFiltered = compSepByCharInSet.joined(separator: "")
        numberFiltered = String(numberFiltered.prefix(allowedCharacterLimit.mobileNumber))
        self.text = numberFiltered
    }
    private func OTPText(){
        let aSet = NSCharacterSet(charactersIn:"0123456789").inverted
        let compSepByCharInSet = self.text!.components(separatedBy: aSet)
        var numberFiltered = compSepByCharInSet.joined(separator: "")
        numberFiltered = String(numberFiltered.prefix(allowedCharacterLimit.otp))
        self.text = numberFiltered
        if self.text?.count == allowedCharacterLimit.otp{
            self.customDelegates?.didEndCharLimit(self)
        }
    }
}
...
extension LMUnderLinedTextField{
    ...
    private func changekeyBoardStyle(){
        switch typeOftextfield {
    ...
    private func changeEditoptions(){
        guard let isenable = isTextFieldEnabled else { return  }
        if isenable{
            editicon.addTarget(self, action: #selector(startEditing) , for: .touchUpInside)
            self.isEnabled = true
            self.rightViewMode = .always
            self.rightView = editicon
            self.rightView?.isHidden = false
            self.rightView?.frame.size = CGSize(width: self.frame.height, height: self.frame.height)
            let showImage =  UIImage(imageLiteralResourceName: "edit_icon")
            editicon.setImage( showImage , for: .normal)
        }else{
    ...
    private func changekeyboardRightview(){
        self.rightView = leftSecureEye
        switch typeOftextfield {
        case .password:
            self.rightViewMode = .always
            leftSecureEye.tintColor = lineColor
            self.rightView = leftSecureEye
            self.rightView?.isHidden = true
            self.rightView?.frame.size = CGSize(width: self.frame.height, height: self.frame.height)
            let showImage =  UIImage(imageLiteralResourceName: "don't_show")
            leftSecureEye.setImage( showImage , for: .normal)
            leftSecureEye.addTarget(self, action: #selector(toggleSecureDataEntry), for: .touchUpInside)
            self.rightView?.isHidden = false
            self.isSecureTextEntry = true
        case .dropDown:
            self.rightViewMode = .always
            dropDown.tintColor = lineColor
            self.rightView = dropDown
            self.rightView?.isHidden = true
            self.rightView?.frame.size = CGSize(width: 15, height: 15)
            let showImage =  UIImage(imageLiteralResourceName: "dropdown")
            dropDown.setImage( showImage , for: .normal)
            dropDown.imageView?.contentMode = .scaleAspectFit
            dropDown.addTarget(self, action: #selector(startEditing) , for: .touchUpInside)
            self.rightView?.isHidden = false
 ...
 @objc private func toggleSecureDataEntry(){
    if self.isSecureTextEntry{
        let showImage = UIImage(imageLiteralResourceName: "show")
        leftSecureEye.setImage( showImage , for: .normal)
    }else{
        let showImage = UIImage(imageLiteralResourceName: "don't_show")
        leftSecureEye.setImage( showImage , for: .normal)
    ...
    private func setFont(){
        ...
        self.font = UIFont.systemFont(ofSize: 16.0) //newLine
    }
}

public struct allowedCharacterLimit{
    static let mobileNumber = 11;
    static let otp = 6;
}
